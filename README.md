...
* 开启 ShadowCaster 和 Receiver:
    * 小米Note
        - SampleAll: 1.333333   1.316667  1.6333333
        - if :             1.33333     1.33333    1.7
        - 6 part :      1.11667     1.1          1.116667

    * S7 edge:
        - SampleAll: 7.9            8.0666        8.08333
        - if:              8.1666      8.15            8.16667
        - 6 part:       3.7666      3.4833        3.83333

    * S9+:
        - SampleAll: 26.86667,    27.2          27.35
        - if:              31.2            31.13         31.35
        - 6 part:       24.9666      25.033       25.033



* 关闭 ShadowCaster 和 Receiver
    * 小米Note
        - SampleAll: 1.61667   1.3              1.31666
        - if :             1.4166     1.33333      1.65
        - 6 part :      1.05         1.05             1.2333

    * S7 edge:
        - SampleAll: 15.883     15.8166      15.7333
        - if:              16.783     16.766        16.766
        - 6 part:       6.2166     6.033          5.85

    * S9+:
        - SampleAll: 47.18333     48.2333     47.6
        - if:              49.21667     49.1666     49.166
        - 6 part:       45.46667     45.3666     45.366

* 开启 ShadowCaster 关闭 Receiver(作为对比，按原理前两个应该帧率一致)
    * 小米Note
        - SampleAll: 1.6333
        - if :             1.7333
        - 6 part :      1.2
    ...
    * S9+:
        - SampleAll: 27.0667       26.5333     27.06667
        - if:              26.8333       27.6333     27.0833
        - 6 part:       25.5166       25.5666    25.1333

## `结论：`
1. 使用 if 合并mesh是最快的，分部件是最慢的
2. 三星S7的 运算量已达瓶颈，增加部件时帧率继续降低，说明drawcall 也已达瓶颈
3.  S9+ 有效 drawcall 为400时帧率可达 49帧，但是分部件并没有大幅度降低帧率，原因是部件被Unity根据材质排序，因此 change state api 较少，此种情况被高通优化。